...
- [src/App.tsx](src/App.tsx) — **toute l'interface dans un seul composant exporté par défaut, ~6 000 lignes.** Les ~16 écrans sont des blocs `{activeTab === 'x' && ...}` en ligne (`dashboard`, `magasins`, `articles`, `stocks`, `inventaires`, `fournisseurs`, `achats`, `receptions`, `affectations`, `transferts`, `employes`, `factures`, `finances`, `users`, `societe`, `sauvegarde`). L'onglet `rapports` (« Analyses & Graphiques ») a été supprimé ; ses exports CSV sont désormais des boutons « Exporter CSV » dans l'en-tête des pages Articles, Magasins, Fournisseurs, Demandes d'Achat et Stocks. Tout l'état est en `useState` en tête de composant ; chaque modale a ses propres états `xModalOpen` + `selectedX` + champs.
